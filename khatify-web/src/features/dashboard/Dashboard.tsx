            ...
            {statsLoading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="border border-border/40 p-4 rounded-lg bg-primary/5">
                    <span className="text-xs text-muted-foreground block mb-1">Paid Invoices</span>
                    <span className="text-lg font-bold text-primary">
                      {stats?.paidInvoicesCount ?? 0}
                    </span>
                  </div>
                  <div className="border border-border/40 p-4 rounded-lg bg-destructive/5">
              ...
              <TableBody>
                {recentLedger.map((entry) => (
                  <TableRow key={entry.id} className="cursor-pointer" onClick={() => navigate(`/customers?id=${entry.customerId}`)}>
                    ...
                    <TableCell>
                      {entry.type === 'debit' ? (
                        <Badge variant="outline" className="text-destructive border-destructive/20 bg-destructive/5 gap-1 py-0.5">
                          <ArrowUpRight className="h-3 w-3" /> Debit
                        </Badge>
                      ) : (
                        <Badge variant="outline" className="text-primary border-primary/20 bg-primary/5 gap-1 py-0.5">
